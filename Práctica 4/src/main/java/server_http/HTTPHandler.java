...
public class HTTPHandler {
    ...
    public HTTPHandler(String request, String method, String resource, String contentType, String contentLength, byte[] body, String timestamp) {
    ...
    // Método para obtener un timestamp con formato sincronizado con NTP
    ...
    public static HTTPHandler readCompleteRequest(DataInputStream input) throws IOException {
        ...
        if(contentLength == -1) return new HTTPHandler(request, method, resource, contentType, contentLengthStr, null, timestamp);
        ...
        return new HTTPHandler(request ,method, resource, contentType, contentLengthStr, body, timestamp);
    }
    ...
    public byte[] getResponse() {
        ...
        if(this.method == HTTPMethod.GET){
            ...
            if (resource.startsWith("/upload"))
                response = Responses.getCreateFileResponse("./resources/public/" + resource.split("/")[2], body, false, this.timestamp);
            else response = Responses.getNotFound();
        }
        else if(this.method == HTTPMethod.PUT){
            if (resource.startsWith("/upload")) 
            response = Responses.getCreateFileResponse("./resources/public/" + resource.split("/")[2], body, true, this.timestamp);
