...
public class Responses {
    ...
    public static byte[] getCreateFileResponse(String filepath, byte[] bytes, boolean isPUT, String timestamp) {
        // Crear archivo
        File file = new File(filepath);
        file.getParentFile().mkdirs();

        try {
            // Si el archivo ya existe, verificar su última modificación
            if (file.exists()) {
                String lastModified = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS")
                        .format(new Date(file.lastModified()));

                // Si el timestamp es más antiguo o igual al último guardado, rechazar la escritura
                if (timestamp.compareTo(lastModified) <= 0) {
                    String jsonresponse = "{\"message\": \"El archivo ya existe y la actualizacion es obsoleta\"}";
            ...
            // Si se permite la escritura (archivo nuevo o actualización válida)
            if (file.createNewFile() || (file.exists() && isPUT)) {
                Files.write(Paths.get(file.getPath()), bytes);
                // Actualizar timestamp del archivo
                file.setLastModified(System.currentTimeMillis());

                String jsonresponse = "{\"message\": \"El archivo fue creado o actualizado\"}";
                ...
                String jsonresponse = "{\"message\": \"No se pudo crear el archivo\"}";
